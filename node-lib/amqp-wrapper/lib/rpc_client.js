...
class RpcClient extends EventEmitter {
  ...
  async init() {
    ;({ queue: this.replyTo } = await this.channel.assertQueue('', {
      exclusive: true,
    }))
  ...
  async send(msg) {
    ...
    debug(
      'sending msg to %s with orrelationId: %s, replyTo: %s',
      this.queueName,
      correlationId,
      this.replyTo
    )
    return new Promise(resolve => {
      this.timeouts[correlationId] = setTimeout(
        () => this.emitter.emit(correlationId, this.gatewayTimeout(msg)),
        this.timeout
      )
