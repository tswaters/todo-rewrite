...
describe('todo integration', () => {
  ...
  it('todo crud operations', async () => {
    ...
    await app
      .post('/api/todo')
      .send({ text: 'learn express' })
      .expect(200)
      .then(res => (todo1 = res.body.todo_id))
    await app
      .post('/api/todo')
      .send({ text: 'learn koa' })
      .expect(200)
      .then(res => (todo2 = res.body.todo_id))
    await app
      .post('/api/todo')
      .send({ text: 'take over the world' })
      .expect(200)
      .then(res => (todo3 = res.body.todo_id))
    ...
    await app
      .post('/api/auth/register')
      .send({ identifier: 'test2', password: 'test' })
      .expect(200)
    await app.get('/api/todo').expect(200, [])
    ...
    await app
      .post(`/api/todo/${todo1}/complete`)
      .send({ complete: true })
      .expect(200)
    await app
      .post(`/api/todo/${todo2}/complete`)
      .send({ complete: true })
      .expect(200)
    await app
      .get('/api/todo')
